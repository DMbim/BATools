...
namespace BA.Ribbon
{
    ...
    public static class RibbonExtensions
    {
        ...
        /// <summary>
        /// Adds pulldown + pulldown + push stacked together (3 items): slots 1 and 2 keep their
        /// own dropdowns, slot 3 is a single plain command. Populate each pulldown's sub-items on
        /// the returned PulldownButtons via pulldown.AddPushButton&lt;TCommand&gt;(...), same as
        /// the other stacked-pulldown helpers in this file. Unlike AddStackedPulldownPushPulldown
        /// above, the push slot sits last instead of in the middle, and unlike
        /// AddStackedPushPulldownPush it takes full small+large icon pairs for every slot since
        /// there was no existing call-site convention to stay backward compatible with here.
        /// </summary>
        public static (PulldownButton top, PulldownButton middle, PushButton bottom) AddStackedPulldownPulldownPush<TCommand3>(
            this RibbonPanel panel,
            string internalName1, string text1, string longDescription1, string smallImagePath1, string largeImagePath1,
            string internalName2, string text2, string longDescription2, string smallImagePath2, string largeImagePath2,
            string internalName3, string text3, string longDescription3, string smallImagePath3, string largeImagePath3)
            where TCommand3 : IExternalCommand
        {
            ...
            void ConfigurePulldown(PulldownButton pd, string longDescription, string smallImagePath, string largeImagePath)
            {
                if (pd == null) return;
                if (!string.IsNullOrWhiteSpace(longDescription))
                    pd.LongDescription = longDescription;
            ...
            ConfigurePulldown(pd1, longDescription1, smallImagePath1, largeImagePath1);
            ConfigurePulldown(pd2, longDescription2, smallImagePath2, largeImagePath2);
            ...
            if (btn3 != null)
            {
                if (!string.IsNullOrWhiteSpace(longDescription3))
                    btn3.LongDescription = longDescription3;
                if (!string.IsNullOrWhiteSpace(smallImagePath3))
                    btn3.SetImage(NormalizePackUri(smallImagePath3));
                if (!string.IsNullOrWhiteSpace(largeImagePath3))
                    btn3.SetLargeImage(NormalizePackUri(largeImagePath3));

                BACommandRegistry.Register(new BACommandInfo
                {
                    InternalName = internalName3,
                    DisplayName = CleanRibbonText(text3),
                    Category = panel.Name ?? "Plugin",
                    FullClassName = typeof(TCommand3).FullName ?? string.Empty,
                    SmallIconPath = smallImagePath3 ?? string.Empty,
                    LargeIconPath = largeImagePath3 ?? string.Empty,
                    ShowInIssueReporter = BACommandRegistry.ShouldShowInIssueReporter(internalName3)
                });
            }
        ...
        public static (PushButton top, PulldownButton middle, PushButton bottom) AddStackedPushPulldownPush<TCommand1, TCommand3>(
            ...
            string internalName1, string text1, string longDescription1, string smallImagePath1,
            string internalName2, string text2, string longDescription2, string smallImagePath2,
            string internalName3, string text3, string longDescription3, string smallImagePath3, string v)
            where TCommand1 : IExternalCommand
            where TCommand3 : IExternalCommand
        {
            var data1 = new PushButtonData(
