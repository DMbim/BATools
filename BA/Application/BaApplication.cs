...
using Autodesk.Revit.UI;        // ── CZA ──
using BA.Addin.Commands;        // ── CZA ──
...
namespace BA.BAApplication
{
    ...
    {
        // ── CZA: singleton bridge, lifetime = Revit session ──────
        public static CzaRevitBridge? CzaBridge { get; private set; }
        ...
        public override void OnStartup()
        {
            ...
            try
            {
                ...
                // ── CZA: ExternalEvent musí vzniknout tady ───────
                var czaDispatcher = System.Windows.Application.Current?.Dispatcher
                       ?? Dispatcher.CurrentDispatcher;
                var czaHandler = new RevitActionQueueHandler(czaDispatcher);
                var czaExternalEvent = ExternalEvent.Create(czaHandler);
                var czaInvoker = new RevitExternalInvoker(czaHandler, czaExternalEvent);
                CzaBridge = new CzaRevitBridge(czaInvoker);
                // ─────────────────────────────────────────────────
                ...
                // ── celý tvůj existující icon/button kód beze změny ──
                ...
                var sCH_16 = "/BA;component/Resources/Icons16/scd_16.png";
                var sCH_32 = "/BA;component/Resources/Icons32/scd_32.png";
                var eVil16 = "/BA;component/Resources/Icons16/evilr_16.png";
                var eVil32 = "/BA;component/Resources/Icons32/evilr_32.png";

                // ── CZA ikony ─────────────────────────────────────
                // Přidej soubory do BA/Resources/Icons16/ a Icons32/
                // nebo použij dočasně existující ikonu jako placeholder
                var czaIcon16 = "/BA;component/Resources/Icons16/areas_16.png";
                var czaIcon32 = "/BA;component/Resources/Icons32/areas_32.png";
                // ─────────────────────────────────────────────────
                ...
                var cmdKeyPln = panelAnnotation.AddPulldownButton<Cmd_KeyplanGridGenerator>(
                    "Set Key Plan", "Set Key\nPlan", "Create Key Plan",
                    kPath16, kPath32);

                cmdKeyPln.AddPushButton<Cmd_KeyplanGridGenerator>(
                    "Generate Keyplan Grids", "KeyPlan\nGrids",
                ...
                var cmdSubCa = panelElements.AddPushButton<Cmd_SubcategoryAuditor>(
                    "SubCat Audit", "SubCat\n Audit",
                    "Create subcategories for all families in the project.",
                    cmClassify16, cmClassify32);
                ...
                var cmdRoomFinishes = panelRooms.AddPushButton<ApplyFinishesByRoomsCommand>(
                    "Room Finishes", "Room\nFinishes",
                    "Calculate finishes for rooms based on room parameters.",
                    fin16, fin32);

                var cmdUse = panelElements.AddPushButton<Cmd_FinishToRoom>(
                    "Finish to Room", "Finish\nto\nRoom",
                    "Automatically finish elements to the level of the room they are in.",
                    use16, use32);

                var arAnno = panelAnnotation.AddPushButton<ArrangeAnnotationsCommand>(
                    "Arrange Annotations", "Anno\nArrange",
                    "Auto Arranges selected annotations",
                    ArAnno16, ArAnno32);

                #region ElementToRoom
                var pdE2R = panelRooms.AddPulldownButton<Cmd_ElementToRoom_Link>(
                    "ElementsToRoom", "Room Nbr\n-> ELements",
                    "Writes RoomNumber into elements located in that room",
                    E2RPath, E2RPath32);

                pdE2R.AddPushButton<Cmd_ElementToRoom_Link>(
                    "ElementToRoomLink", "Link",
                    "Writes information into elements based on location (linked room)",
                    cmE2RLink16, cmE2RLink32);

                pdE2R.AddPushButton<Cmd_ElementToRoom_Local>(
                    "ElementToRoomLocal", "Local",
                    "Writes information into elements based on location (room)",
                    cmE2RLocal16, cmE2RLocal32);
                #endregion
                ...
                var pdDim = panelAnnotation.AddPulldownButton<Cmd_DimensionEdit>(
                    "DimEditPulldown", "Dim\nEdit",
                    "Mass Dimension Edit",
                    cmDimOverride16, cmDimOverride32);
                ...
                panelManagement.AddPushButton<Cmd_SheetDateAndRevision>(
                    ...
                    cmSheetRenumber16, cmSheetRenumber32);

                #region Management
                panelManagement.AddPushButton<Cmd_FamilyParameters>(
                    "HarmonizeFamilyParameters", "Harmonize\nFamily Params",
                ...
                var pdMon = panelManagement.AddPulldownButton<Cmd_ChangeMonitorStart>(
                    ...
                    "Track adds, deletes, moves and parameter edits.",
                    cmIcon16, cmIcon32);
                ...
                pdAxis.AddPushButton<Cmd_AxisToRoom_Link>(
                    "AxisToRoomLink", "Link",
                    "Pick room tags hosted in the model, resolve rooms in linked model.",
                    cmA2RLink16, cmA2RLink32);

                pdAxis.AddPushButton<Cmd_AxisToRoom_Local>(
                    "AxisToRoomLocal", "Local",
                    "Pick room tags in active model, resolve rooms locally.",
                    cmA2RLocal16, cmA2RLocal32);
                #endregion

                panelElements.AddPushButton<Cmd_RayBounceCeiling>(
                    "RayBounceCeiling", "Ray Bounce\nCeiling",
                ...
                // ── CZA: přidat na konec panelRooms ──────────────
                var pdCza = panelRooms.AddPulldownButton<OpenMainWindowCommand>(
                    "CzechAreas",
                    "Plochy\nCZ",
                    "Výpočet ploch dle české legislativy (NV 366/2013, PSP, SZ)",
                    czaIcon16,
                    czaIcon32);

                pdCza.AddPushButton<OpenMainWindowCommand>(
                    "CzaOpenWindow",
                    "Výpočet ploch",
                    "Otevře panel pro výpočet podlahové plochy (NV 366/2013), " +
                    "HPP nadzemní/podzemní (PSP §2c, §2g), " +
                    "podlahové plochy (SZ §13n) a zastavěné plochy (SZ §13o).",
                    czaIcon16,
                    czaIcon32);

                pdCza.AddPushButton<InitializeParametersCommand>(
                    "CzaInitParams",
                    "Init parametry",
                    "Vytvoří a sváže sdílené parametry CZA v aktuálním dokumentu. " +
                    "Spusť jednou při prvním použití na novém modelu.",
                    czaIcon16,
                    czaIcon32);
                // ─────────────────────────────────────────────────
            }
