...
namespace BA.RoomClassification
{
    ...
    public sealed class RoomClassificationImportCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            using (TransactionGroup tg = new TransactionGroup(doc, "Import Room Classification"))
            {
                ...
                IList<RoomClassificationParameterDefinition> parameterDefinitions = RoomClassificationParameterCatalog.BuildDefault();

                using (Transaction t1 = new Transaction(doc, "Ensure room shared parameters"))
                {
                    t1.Start();
                    RoomClassificationSharedParameterService.EnsureRoomParameters(uiApp.Application, doc, parameterDefinitions);
                    t1.Commit();
                }
                ...
                {
                    t2.Start();
                    keySchedule = RoomClassificationScheduleService.EnsureRoomKeySchedule(doc, parameterDefinitions);
                    t2.Commit();
                }
                ...
                {
                    t3.Start();
                    syncResult = RoomClassificationSyncService.UpsertRoomClassificationKeys(doc, keySchedule, records);
                    t3.Commit();
                }
    ...
    internal static class RoomClassificationValidator
    {
        public static RoomClassificationValidationResult Validate(IReadOnlyList<RoomClassificationRecord> records)
        {
            ...
            foreach (IGrouping<string, RoomClassificationRecord> grp in records.GroupBy(x => x.RoomCode, StringComparer.OrdinalIgnoreCase))
            {
                if (grp.Count() > 1)
                    result.Errors.Add($"Duplicate BA_RoomCode '{grp.Key}' in rows {string.Join(", ", grp.Select(x => x.SourceRowNumber))}.");
            }

            foreach (IGrouping<string, RoomClassificationRecord> grp in records.GroupBy(x => x.RoomKey, StringComparer.OrdinalIgnoreCase))
            {
                if (grp.Count() > 1)
                    result.Errors.Add($"Duplicate BA_RoomKey '{grp.Key}' in rows {string.Join(", ", grp.Select(x => x.SourceRowNumber))}.");
            }

            return result;
        }

        private static void Require(RoomClassificationRecord record, string field, string value, RoomClassificationValidationResult result)
        {
            if (string.IsNullOrWhiteSpace(value))
    ...
    {
        public static void EnsureRoomParameters(Autodesk.Revit.ApplicationServices.Application app, Document doc, IList<RoomClassificationParameterDefinition> parameterDefinitions)
        {
            ...
            try
            {
                ...
                foreach (RoomClassificationParameterDefinition p in parameterDefinitions)
                {
                    ...
                    if (!inserted && !doc.ParameterBindings.ReInsert(def, binding, GroupTypeId.Data))
                        throw new InvalidOperationException($"Failed to bind parameter '{p.Name}' to Rooms.");
                }
            }
            finally
            {
        ...
        private static void WriteTemporarySharedParameterFile(string filePath, IList<RoomClassificationParameterDefinition> parameterDefinitions)
        {
            StringBuilder sb = new StringBuilder();
    ...
    internal static class RoomClassificationScheduleService
    {
        ...
        public static ViewSchedule EnsureRoomKeySchedule(Document doc, IList<RoomClassificationParameterDefinition> parameterDefinitions)
        {
            ViewSchedule existing = new FilteredElementCollector(doc)
                .OfClass(typeof(ViewSchedule))
                .Cast<ViewSchedule>()
                .FirstOrDefault(x => !x.IsTemplate && string.Equals(x.Name, ScheduleName, StringComparison.OrdinalIgnoreCase));

            ViewSchedule schedule = existing ?? ViewSchedule.CreateKeySchedule(doc, new ElementId(BuiltInCategory.OST_Rooms));
        ...
        private static void EnsureFields(Document doc, ViewSchedule schedule, IList<RoomClassificationParameterDefinition> parameterDefinitions)
        {
            ...
            foreach (RoomClassificationParameterDefinition p in parameterDefinitions)
            {
                ...
                SchedulableField sf = schedulableFields.FirstOrDefault(x => string.Equals(x.GetName(doc), p.Name, StringComparison.OrdinalIgnoreCase));
                if (sf != null)
    ...
    {
        public static RoomClassificationSyncResult UpsertRoomClassificationKeys(Document doc, ViewSchedule keySchedule, IReadOnlyList<RoomClassificationRecord> sourceRows)
        {
            RoomClassificationSyncResult result = new RoomClassificationSyncResult();
            Dictionary<string, Element> existingByCode = CollectExistingKeyElementsByCode(doc, keySchedule);
            ...
            HashSet<string> importedCodes = new HashSet<string>(sourceRows.Select(x => x.RoomCode), StringComparer.OrdinalIgnoreCase);
        ...
        private static Dictionary<string, Element> CollectExistingKeyElementsByCode(Document doc, ViewSchedule keySchedule)
        {
            Dictionary<string, Element> result = new Dictionary<string, Element>(StringComparer.OrdinalIgnoreCase);

            foreach (Element e in new FilteredElementCollector(doc, keySchedule.Id).WhereElementIsNotElementType().ToElements())
            {
        ...
        private static Element CreateNewKeyElement(Document doc, ViewSchedule keySchedule)
        {
            ...
            if (insertAt < body.FirstRowNumber)
                insertAt = body.FirstRowNumber;
            ...
            if (newId == null || newId == ElementId.InvalidElementId)
                throw new InvalidOperationException("Could not resolve the new key element after inserting a row.");

            Element newElement = doc.GetElement(newId);
            if (newElement == null)
                throw new InvalidOperationException("The new key element could not be resolved from the document.");
    ...
    internal static class ParameterWriteUtil
    {
        ...
        public static void SetString(Element element, string parameterName, string value)
        {
            Parameter p = FindParameter(element, parameterName);
            if (p == null)
                throw new InvalidOperationException($"Parameter '{parameterName}' was not found on element {element.Id.Value}.");

            if (p.IsReadOnly)
                throw new InvalidOperationException($"Parameter '{parameterName}' is read-only on element {element.Id.Value}.");

            if (p.StorageType != StorageType.String)
                throw new InvalidOperationException($"Parameter '{parameterName}' is not a text parameter on element {element.Id.Value}.");
