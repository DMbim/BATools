    ...
    public class Cmd_ChangeMonitorStop : ExternalCommand
    {
        ...
        private static void ExportCsv(ChangeReport report)
        {
            ...
            foreach (var r in report.Records)
            {
                ...
                {
                    sb.AppendLine($"{r.When:O},{r.ChangeTypeDisplay},{Csv(elId)},{Csv(r.Category)},{Csv(r.ViewName)},{Csv(vId)},{Csv(r.Username)},{Csv(r.TransactionNames)},,,");
                }
                else
                {
                    ...
                    {
                        sb.AppendLine(
                            $"{r.When:O},{r.ChangeTypeDisplay},{Csv(elId)},{Csv(r.Category)},{Csv(r.ViewName)},{Csv(vId)},{Csv(r.Username)},{Csv(r.TransactionNames)},{Csv(p.ParamName)},{Csv(p.OldValue)},{Csv(p.NewValue)}");
                    }
                }
            }
