...
namespace BA.Commands.Families
{
    ...
    public class Cmd_ReportFamilySubcategories : IExternalCommand
    {
        ...
        public static Result Run(UIApplication uiApp, ref string message)
        {
            ...
            try
            {
                if (projectDoc == null)
                {
                    TaskDialog.Show("Family Subcategories", "No active document.");
    ...
    public static class FamilySubcategoryScanner
    {
            ...
            FamilySubcategoryScanOptions options)
        {
            ...
            {
                if (family == null) { result.FamiliesSkipped++; continue; }
        ...
        private static void CollectRowsFromFamilyDocument(
            ...
            FamilySubcategoryScanResult result)
        {
            if (familyDoc == null || !familyDoc.IsFamilyDocument) return;

            Family ownerFamily = familyDoc.OwnerFamily;
            if (ownerFamily == null) return;

            Category familyCategory = ownerFamily.FamilyCategory;
            if (familyCategory == null) return;
        ...
        public static bool IsSupportedEditableFamily(Document projectDoc, Family family)
        {
            ...
            try { if (!family.IsEditable) return false; }
            catch { return false; }
            return true;
        }
        ...
        public static bool StartsWithPrefix(string name, string prefix)
        {
            ...
            return (name ?? string.Empty).StartsWith(prefix, StringComparison.OrdinalIgnoreCase);
        }
        ...
        public static string SafeCategoryName(Category c)
        {
            ...
            try { return c.Name ?? string.Empty; } catch { return string.Empty; }
        }
        ...
        public static string SafeFamilySourceInfo(Family family)
        {
            ...
            try { return family.IsInPlace ? "InPlace" : "Loadable"; } catch { return string.Empty; }
        }

        public static int ToInt(ElementId id)
        {
            if (id == null) return -1;
            try { return unchecked((int)id.Value); } catch { return -1; }
        }
    }
    ...
    public static class FamilySubcategoryReportWriter
    {
            ...
            string baseFileName)
        {
            ...
            sb.AppendLine(string.Join(",",
                Csv("ProjectTitle"), Csv("FamilyName"), Csv("FamilyId"),
                Csv("FamilyCategoryName"), Csv("FamilyCategoryId"),
                Csv("ParentCategoryName"), Csv("ParentCategoryId"),
                Csv("SubcategoryName"), Csv("SubcategoryId"),
                Csv("StartsWithPrefix"), Csv("IsRootCategoryRow"),
                Csv("IsEditableFamily"), Csv("SourceFamilyPathOrInfo")));
            ...
            {
                sb.AppendLine(string.Join(",",
                    Csv(row.ProjectTitle), Csv(row.FamilyName),
                    Csv(row.FamilyId.ToString()), Csv(row.FamilyCategoryName),
                    Csv(row.FamilyCategoryId.ToString()), Csv(row.ParentCategoryName),
                    Csv(row.ParentCategoryId.ToString()), Csv(row.SubcategoryName),
                    Csv(row.SubcategoryId.ToString()), Csv(row.StartsWithPrefix.ToString()),
                    Csv(row.IsRootCategoryRow.ToString()), Csv(row.IsEditableFamily.ToString()),
        ...
        private static string GetReportFolder(Document projectDoc)
        {
            ...
            try { docPath = projectDoc.PathName ?? string.Empty; } catch { }
